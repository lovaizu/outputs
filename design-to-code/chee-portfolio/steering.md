...
<!-- active: 2026-05-13 — next: #12.1 Header（画像ベース再検証） -->
...
1. カンプ画像からUI要素チェックリスト作成（配置・サイズ・色・形状など全プロパティを画像から算出）
2. UX/UIデザインエキスパートがカンプ画像＋チェックリストをレビュー → 指摘ゼロまで改善
3. チェックリスト通りに実装修正
4. Playwrightスクショ＋チェックリストでUX/UIデザインエキスパートが検証 → 指摘ゼロまで改善
