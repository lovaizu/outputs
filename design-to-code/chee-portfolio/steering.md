...
<!-- paused: 2026-05-14 — next: #12.2 FV ステップ5 ユーザーレビュー続行（jitter・速度のブラウザ実機確認残） -->
...
- **Current state: 12.2 FV ステップ5 ユーザーレビュー中。残: jitter・速度のブラウザ実機確認**
- **FV 修正済み:**
  - 背景色F6F6F6統一、identity block削除、Chee Design文字色グラデーション
  - 角丸: `border-radius: 0 80px 80px 0`（ユーザー指定）
  - 青エリア右マージン: 250px（"Chee Design" の "e" 右端付近）
  - モックループ: autoWidth + 画像106px固定、ページ右端まで表示（`.fv-lower__inner` width: calc(100% + 250px)）
  - スタガー: transform→margin-top に変更 + will-change/backface-visibility でGPU昇格（jitter対策）
  - data-splide-index でクローンスライドにも一貫したスタガー適用
  - 速度: 0.5→1.2
