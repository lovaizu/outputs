...
evaluator が「instruction が独立性の唯一の機構」として EA.1 違反を指摘。T1 事実確認（cc-rules-redesign/steering.md T1 完了メモ）で Agent tool のサブエージェントは各呼び出しで独立したコンテキストを持ち、親の会話履歴に構造的にアクセスできないことを確認済み。finding は誤った前提に基づいていたため **reject**（3 evaluators 全員合意）。
...
**Evidence**: T2–T5 の5ラウンド評価で、この制約下でも evaluator は valid findings を返した（6件解消、escalation 1件）。停止を適用した場合、assay.md が Step 2 で読み込む `action.md` と `evaluation.md` の2ファイルが評価対象外になる。この制約は assay.md Step 3 末尾に Known Constraint として明示済み（assay.md line 49）。
